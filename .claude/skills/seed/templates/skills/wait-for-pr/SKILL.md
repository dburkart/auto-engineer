...
2. Count review-bot activity:
   ```sh
   gh api "repos/{{GITHUB_OWNER}}/{{GITHUB_REPO}}/issues/<PR>/comments" \
...
|---|---|---|
| format check | `{{FORMAT_CMD}}` | `{{FORMAT_FIX_COMMIT}}` |
| lint check | `{{LINT_FIX_CMD}}` | `{{LINT_FIX_COMMIT}}` |
...
# format:
git log "origin/${BASE_REF}..HEAD" --format='%s' | grep -Fxq '{{FORMAT_FIX_COMMIT}}'
# lint:
git log "origin/${BASE_REF}..HEAD" --format='%s' | grep -Fxq '{{LINT_FIX_COMMIT}}'
```
